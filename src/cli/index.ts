...
import { registerAuditSuite } from './commands/audit-suite'
...
program
  ...
  .option('--json', 'Emit JSON', false)
  .action(async (options) => { process.exit(await selfCheckCommand(options)) })
...
program
  ...
  .option('--run-tests', 'T-D1 close — spawn runner on tests/<feature>/ + read counts automatically', false)
  .option('--runner <name>', 'Runner: vitest | jest | playwright (default: vitest)')
...
program
  .command('perf')
  .description('T-010 — Performance budget evaluator + Lighthouse runner + trend + CI PR comment')
  ...
  .option('--lighthouse', 'Run Lighthouse (via npx) on --urls and emit runs JSON', false)
  .option('--urls <csv>', 'Comma-separated URLs for --lighthouse')
  .option('--lighthouse-flags <flags>', 'Extra CLI flags passed through to lighthouse')
  .option('--out <path>', 'Output path for --lighthouse runs JSON')
  .option('--trend', 'With --lighthouse, append the runs to coverage/perf-trend.jsonl', false)
  .option('--trend-report', 'Read trend JSONL, render week-over-week changes', false)
  .option('--post-pr', 'Post the CI comment to the PR (needs GITHUB_TOKEN)', false)
  .option('--pr-marker <text>', 'Idempotency marker used to find + replace prior comment')
  .option('--owner <name>', 'GitHub repo owner (defaults to GITHUB_REPOSITORY)')
  .option('--repo <name>', 'GitHub repo (defaults to GITHUB_REPOSITORY)')
  .option('--pr-number <n>', 'PR number (defaults to GitHub Actions event payload)', (v) => parseInt(v, 10))
  .option('--json', 'Emit JSON', false)
